...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'logs') {
      ...
      const rows = await sql`SELECT * FROM pagos_historial WHERE contract_id = ${contract_id} ORDER BY executed_at DESC`;
    ...
    if (req.method === 'POST') {
      ...
      if (action === 'add') {
        ...
        await sql`
          INSERT INTO pagos_historial (contract_id, payment_amount, payment_number) 
          VALUES (${contract_id}, ${amount}, ${payment_number})
        `;
        await sql`
          UPDATE contratos SET payments_made = payments_made + 1, updated_at = CURRENT_TIMESTAMP WHERE id = ${contract_id}
        `;
        return res.json({ message: 'Pago registrado exitosamente' });
      }

      if (action === 'liquidate') {
        await sql`UPDATE contratos SET status = 'LIQUIDATED', updated_at = CURRENT_TIMESTAMP WHERE id = ${contract_id}`;
